...
$defaultOutputPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\dist\pkg_treek_free.zip'))
$outputFullPath = [System.IO.Path]::GetFullPath($OutputPath)
$buildOutputPath = $outputFullPath

if ($outputFullPath -ieq $defaultOutputPath) {
    $buildOutputPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot ('..\pkg_treek_free.' + [guid]::NewGuid().ToString('N') + '.publish.zip')))
}
...
& (Join-Path $PSScriptRoot 'build-common.ps1') `
    ...
    if ($buildOutputPath -ine $outputFullPath) {
        ...
        Write-Output "Published TreeK Free package ZIP: $outputFullPath"
    }
} finally {
