...
уже содержит `user_id`, `context` и JSON-поле `settings`. Для текущей модели
TreeK на одного пользователя должна быть одна строка `context = default`.
Настройки дерева и настройки вмешательства в Kunena/template хранятся в одном
JSON, но в разных разделах.
...
Старый экспериментальный контекст `treek_view` не используется: при обновлении
его значения сливаются в `settings.treekViewFeatures`, а лишняя строка удаляется.
