...
уже содержит `user_id` и JSON-поле `settings`. Для текущей модели TreeK на
одного пользователя должна быть одна строка. Настройки дерева и настройки
вмешательства в Kunena/template хранятся в одном JSON, но в разных разделах.

Флаги вмешательства в Kunena/template хранятся внутри:

```text
settings.treekViewFeatures
```

Отдельный контекст `treek_view` не используется.

Глобальные значения TreeKView хранятся отдельно:

```text
#__treek_global_parameters
```

Они работают как дефолты для гостей и для зарегистрированных пользователей, у
которых ещё нет сохранённых `settings.treekViewFeatures`. Пользовательская
настройка сильнее глобальной: если у пользователя есть своё значение флага, оно
переопределяет значение администратора.

Начальные глобальные значения:
...
`debug_ajax` также хранится в глобальной таблице. Если строка ещё отсутствует,
плагин использует старый параметр Joomla-плагина как fallback.
...
Статус: подключён к визуальной части reply-форм и subject-полю ответов. Если
флаг включён, заголовок полной reply-формы выводится как TreeK-вариант `h5` с
HTML-ссылкой автора. Если флаг выключен, используется `h1` с plain-текстом из
`headerText`, чтобы HTML-ссылка автора не выводилась как буквальный текст.

Для reply subject действует отдельное правило удобства: если
`reply_form_treek_look` включён, а `subject_suffix` выключен, subject-поле ответа
открывается пустым, чтобы пользователь сам ввёл конкретный заголовок поста. Если
`subject_suffix` включён, он имеет приоритет: поле остаётся под управлением
`treek_subject.js`, открывается пустым и получает защищённый суффикс через
`data-treek-suffix`.
...
группам: `subject_suffix` и `reply_form_treek_look`. При дальнейшей детализации
важно сохранять приоритет `subject_suffix` над очисткой поля, которую включает
`reply_form_treek_look`.
...
Такого helper пока нет. Настройки вмешательства в Kunena/template хранятся в
разделе `settings.treekViewFeatures` пользовательского JSON.
